...
- `grep` 的 PATTERN：优先作正则；非法正则报错。字面量请用 `-F`（如 `grep -F 'F:\mingw'`）
- 无通配符的字面路径行为与原先一致（不存在仍由各命令报错）
- 多数命令通过 `$LASTEXITCODE` 返回 Unix 风格退出码（0 成功 / 1 未匹配或有差异 / 2 用法错误）
...
接入管道时：非 `-l` 输出**文件名**；带 `-l`/`-ahl` 时输出完整长列表文本行，便于 `ls -l | grep`。
...
find [路径...] [-name 模式] [-iname 模式] [-type f|d|l]
     [-maxdepth N] [-mtime N] [-mmin N] [-atime N] [-ctime N] [-size 规格]
```
...
| `-type d` | 目录 |
| `-type l` | 符号链接 |
| `-maxdepth N` | 限制相对根路径的最大深度（0 仅根本身） |
| `-mtime N` | 按修改时间（天）过滤 |
| `-mmin N` | 按修改时间（分钟）过滤 |
| `-atime N` | 按访问时间（天）过滤 |
...
按正则表达式匹配文本行。可从文件读取，也可从管道接收；无文件时读标准输入。

匹配片段在终端显示时标红。退出码：0 有匹配，1 无匹配，2 错误。
...
|------|------|
| `-r` / `-R` | 递归删除目录 |
| `-f` | 强制：覆盖只读、路径不存在时不报错；**仅 `-f` 时才 Force** |
| 组合 | `-rf`、`-fr` 等 |
...
估算文件或目录占用的磁盘空间。默认**递归列出各子目录**（对齐 GNU `du`）；`-s` 只输出总计。
